...
eww_bin = [
    subprocess.getoutput("which eww"),
    "-c",
    f"{os.path.expanduser('~')}/.config/eww_sway/sidebar",
]
...
def update_log(updated, recent):
    subprocess.run(
        [
            ...
            eww_bin[2],
            "update",
            f"notifications={json.dumps(updated)}",
        ]
    )
    subprocess.run(
        [
            ...
            eww_bin[2],
            "update",
            f"newnotification={json.dumps(recent)}",
        ]
    )
...
def dismiss():
    ...
    subprocess.run(
        [
            ...
            eww_bin[2],
            "update",
            f"newnotification={json.dumps(blank)}",
        ]
    )
    ...
    if argv[1] == "listen":
        proc = subprocess.Popen(
            [
                ...
                "-s",
                "-o",
                '{"source": "#source", "icon": "#icon", "summary": "#summary", "body": "#body", "action": "#actions", "timeout": #timeout}',
            ],
