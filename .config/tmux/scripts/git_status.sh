...
cd "$1" || exit 1
RESET="#[fg="#2D4F67",bg="#16161D",nobold,noitalics,nounderscore,nodim]"
...
if [[ $CHANGED_COUNT -gt 0 ]]; then
  STATUS_CHANGED="${RESET}#[fg="#DCA561",bg="#16161D",bold] ${CHANGED_COUNT} "
fi

if [[ $INSERTIONS_COUNT -gt 0 ]]; then
  STATUS_INSERTIONS="${RESET}#[fg="#76946A",bg="#16161D",bold] ${INSERTIONS_COUNT} "
fi

if [[ $DELETIONS_COUNT -gt 0 ]]; then
  STATUS_DELETIONS="${RESET}#[fg="#C34043",bg="#16161D",bold] ${DELETIONS_COUNT} "
fi

if [[ $UNTRACKED_COUNT -gt 0 ]]; then
  STATUS_UNTRACKED="${RESET}#[fg="#363646",bg="#16161D",bold] ${UNTRACKED_COUNT} "
fi
...
case "$SYNC_MODE" in
1)
  REMOTE_STATUS="$RESET#[bg="#16161D",fg="#6A9589",bold] 󱓎"
  ;;
2)
  REMOTE_STATUS="$RESET#[bg="#16161D",fg="#938056",bold] 󰛃"
  ;;
3)
  REMOTE_STATUS="$RESET#[bg="#16161D",fg="#E46876",bold] 󰛀"
  ;;
*)
  REMOTE_STATUS="$RESET#[bg="#16161D",fg="#98BB6C",bold] "
  ;;
esac
